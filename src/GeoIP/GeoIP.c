...
unsigned int _GeoIP_seek_record_v6 (GeoIP *gi, geoipv6_t ipnum) {
        ...
        * These begin with 2002: or 2001:0:, so they are fairly easy prefixes to
        * check, but we have to deal with it in a way that deals with endian issues,
        * so it's not all that pretty.  Sadly, we don't seem to have portable
        * macros like htonl for this v6 funtion just yet.  Using a cast to uint32_t
        * so that we get the first 4 bytes of the address.
        */
       ...
       if ((((__const uint32_t *) (ipnum.s6_addr))[0] & 0xffff0000) == 0x20020000)
          off = 2;
       else if ((((__const uint32_t *) (ipnum.s6_addr))[0] & 0xffffffff) == 0x20010000)
          off = 4;
#else /* _BIG_ENDIAN */
       if ((((__const uint32_t *) (ipnum.s6_addr))[0] & 0x0000ffff) == 0x00000220)
          off = 2;
       else if ((((__const uint32_t *) (ipnum.s6_addr))[0] & 0xffffffff) == 0x00000120)
          off = 4;
#endif /* _BIG_ENDIAN */
       ...
       if (off) {
         ...
         /*
          * The v6 address field is uint8_t[16], and the first 2 or 4 members of that
          * array are taken up the 2002: or 2001:0: prefix, respectively.  We want to
          * iterate the next 4, as that will get us the mapped v4 address back.  We
          * don't have to worry about endian issues here, as the rest of the code seems
          * to keep ip addresses in whichever form is native to the architecture.
          */

         for (part = off; part <= (off + 3); part++) {
           v4.s_addr <<= 8;
